...
figure(15)
clf
lim_ = abs(max(U_avg_rd) - min(U_avg_rd)); % por defecto estaba con smith (una s en vez de rd)
subplot(2,1,1)
%subplot('Position',  [0.1 0.1 0.5 0.5] )
plot(T,U_avg_rd, '--')
hold on
...
ylim([min(U_avg_rd)-lim_*.1 max(U_avg_rd)+lim_*.2])% por defecto estaba con smith (una s en vez de rd)
xlim([T(1) T(end)])
ylabel('Utility')
xlabel('Time')


h = legend('Replicator Dynamics','Location','northoutside'); % 'Logit Dynamics', 'Smith Dynamics', 'BNN Dynamics',4);
set(h,'Interpreter','none')
...
lim_ = abs(max(P_avg_rd) - min(P_avg_rd));% por defecto estaba con smith (una s en vez de rd)
subplot(2,1,2)
%axes('Position',[0.1 0.1 0.5 0.5])
pb=plot(T,P_avg_rd, '--','LineWidth',2);
%hold on
%pd = plot(T,P_avg, 'g--','LineWidth',2);
%pa=plot(T,P_avg_s, 'k');
%pc = plot(T, P_avg_bnn, 'r-.');
...
%ylim([min(P_avg_s)-lim_*.1 max(P_avg_s)+lim_*.2]) Modificada si solo se
%calcula rd
ylim([min(P_avg_rd)-lim_*.1 max(P_avg_rd)+lim_*.2])
ylabel('Power')
%h = legend( 'Logit Dynamics', 'Replicator Dynamics', 'Smith Dynamics', 'BNN Dynamics',1);
%set(h,'Interpreter','none')
xlabel('Time')
...
lim_ = abs(max(I_avg_rd) - min(I_avg_rd));% por defecto estaba con smith (una s en vez de rd)
%subplot(3,1,3)
p4 = plot(T,I_avg_rd, '--','LineWidth',2);
%hold on
%p2 = plot(T,I_avg, 'g--');
%p1 = plot(T,I_avg_s, 'k');
%p3 = plot(T,I_avg_bnn, 'r-.');
...
ylabel('Incentives')
xlabel('Time')
h = legend(  'Replicator Dynamics', 'Location','northoutside'); % 'Logit Dynamics','Smith Dynamics', 'BNN Dynamics',2);
set(h,'Interpreter','none')
title('Average Incentives of the Society')
%set(h, 'FontSize', 20)
xlim([T(1) T(end)])
...
%p4=plot(T, int_logit./int_avg, 'g--','LineWidth',2);
%hold on
p2=plot(T, int_replicator, '--','LineWidth',2); % ./int_avg, '--','LineWidth',2);
%p1=plot(T, int_smith./int_avg, 'k');
...
ylabel('Accumulated Incentives')
xlabel('Time')
h = legend( 'Replicator Dynamics','Location','northoutside'); % 'Logit Dynamics', 'Replicator Dynamics', 'Smith Dynamics', 'BNN Dynamics',4);
set(h,'Interpreter','none')
